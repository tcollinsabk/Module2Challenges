...
void GameplayState::HandleCollision(int newPlayerX, int newPlayerY)
{
	...
	if (collidedActor != nullptr && collidedActor->IsActive())
	{
		...
		case ActorType::Dog:
		{
			Dog* collidedDog = dynamic_cast<Dog*>(collidedActor);
			assert(collidedDog);
			if (!m_player.HasKey(collidedDog->GetColor()))
			{
				AudioManager::GetInstance()->PlayLoseLivesSound();
				collidedDog->Remove();
				m_player.SetPosition(newPlayerX, newPlayerY);
			...
			else if (m_player.HasKey(collidedDog->GetColor()))
			{
				m_player.UseKey();
				collidedDog->UpdateFriendly(true);
			}
			else if (collidedDog->IsFriendly())
			{
